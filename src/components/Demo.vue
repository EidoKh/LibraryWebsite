...
function afterTransition(callback) {
  const listener = () => {
    callback();
    // this.$ref.inner.removeEventListener("transitioned", listener);
    document
      .getElementsByClassName(".inner")
      .removeEventListener("transitioned", listener);
  };
  //   this.$ref.inner.addEventListener("transitioned", listener);
  document
    .getElementsByClassName(".inner")
    .addEventListener("transitioned", listener);
}
function moveLeft() {
  console.log("moveLeft");
  innerStyles.value = { transform: `translateX(-${step})` };
}
</script>
<style>
.carousel {
